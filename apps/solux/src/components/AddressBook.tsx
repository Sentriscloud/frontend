    ...
    <div className="min-h-screen flex justify-center px-5 py-8">
      <div className="w-full max-w-sm">
        ...
        <div className="flex items-end justify-between mb-6 animate-fade-up delay-1">
          <div>
            <div className="eyebrow">Saved recipients</div>
            <h1 className="font-serif text-3xl text-[var(--tx)] mt-1">Address book</h1>
          </div>
          <button
            onClick={() => setShowAdd(true)}
            className="w-10 h-10 rounded-lg flex items-center justify-center bg-[var(--gold)] text-[var(--bk)] hover:bg-[var(--gold-l)] transition-colors active:scale-[0.99]"
          >
          ...
          {entries.length === 0 ? (
            <div className="p-10 text-center">
              <div className="w-12 h-12 rounded-xl mx-auto mb-3 flex items-center justify-center bg-[var(--bk-2)] border border-[var(--brd)]">
                <BookOpen className="w-5 h-5 text-[var(--tx-d)]" />
              </div>
              <p className="text-sm text-[var(--tx-2)]">No saved recipients</p>
              <p className="text-[11px] text-[var(--tx-d)] mt-1">Tap + to add an address with a label</p>
            </div>
          ) : (
            <div className="divide-y divide-[var(--brd)]">
              {entries.map((e) => (
                <div key={e.address} className="px-4 py-3 flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <p className="text-sm text-[var(--tx)]">{e.label}</p>
                    <p className="text-[10px] font-mono text-[var(--tx-d)] truncate mt-0.5">{truncate(e.address)}</p>
                    {e.note && (
                      <p className="text-[11px] text-[var(--tx-m)] mt-1 italic">{e.note}</p>
                    )}
                  </div>
                  <div className="flex items-center gap-1 shrink-0">
                    <button
                      ...
                      aria-label="Edit"
                      className="w-8 h-8 rounded-md flex items-center justify-center hover:bg-[var(--sf-2)] transition-colors"
                    >
                    ...
                    <button
                      ...
                      aria-label="Delete"
                      className="w-8 h-8 rounded-md flex items-center justify-center hover:bg-[var(--red-bg)] transition-colors"
                    >
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/70">
          <div className="w-full max-w-sm rounded-t-2xl sm:rounded-2xl overflow-hidden bg-[var(--sf)] border border-[var(--brd)] animate-fade-up">
            <div className="flex items-center justify-between px-5 pt-5 pb-2">
              <div>
                <div className="eyebrow">{editing ? 'Update entry' : 'New entry'}</div>
                <h2 className="font-serif text-lg text-[var(--tx)]">
                  {editing ? 'Edit recipient' : 'Add recipient'}
                </h2>
              </div>
              <button onClick={reset} aria-label="Close" className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-[var(--sf-2)] transition-colors -mr-1">
            ...
            <div className="px-5 pb-6 space-y-4">
              ...
              <div>
                <label className="eyebrow block mb-2">Note (optional)</label>
                <input
                  value={note}
