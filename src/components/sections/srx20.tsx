...
const SRX20_FEATURES = [
  ...
  },
  {
    ...
    title: "Gas in SRX",
    desc: "Every SRX-20 transaction pays gas in SRX — driving constant demand for the native coin.",
  },
  {
...
export function SRX20() {
  return (
    <section id="srx20" className="srx20-section py-[120px] px-6 md:px-[60px] border-t border-b border-[var(--brd)]">
      <Reveal>
        <SectionHeader tag="Token Standard" title="SRX-20" titleEm="." subtitle="The Sentrix token standard. Deploy your own token on Sentrix Chain — permissionless, instant, and gas-efficient." />
      </Reveal>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-start">
        ...
        <div className="group relative bg-gradient-to-br from-[var(--sf)] to-[var(--bk)] border border-[var(--brd)] p-8 overflow-hidden rounded-2xl hover:border-[rgba(167,139,250,.15)] transition-all duration-500">
          ...
          <span className="absolute top-3.5 right-4 font-mono text-[8px] tracking-[.2em] px-2 py-1 rounded-md" style={{ color: "var(--teal)", border: "1px solid rgba(45,212,191,.15)", background: "rgba(45,212,191,.05)" }}>SRX-20</span>
          <pre className="font-mono text-[12px] leading-[1.9] text-[var(--tx-m)] overflow-x-auto">
            <code>
              <span className="text-[var(--tx-d)]"># Deploy your SRX-20 token</span>{"\n\n"}
              ...
              <span className="text-[var(--cyan)]">Contract:</span> <span className="text-[var(--green)]">SRX20_2be9edc653...</span>{"\n"}
        ...
        <div className="flex flex-col gap-3">
          {SRX20_FEATURES.map((f, i) => {
            const c = SRX20_COLORS[i];
            return (
